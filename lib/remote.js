...
var stats = {
  ...
  // cpu  : "top -bn 2 -d 0.3 | grep '^%\\?Cpu.s.' | awk '{print $2+$4+$6}' | awk '{sum+=$1} END {print sum/NR \"%\" }'q",
  cpu  : "top -bn 2 -d 0.3 | grep '^%\\?Cpu.s.' | tail -1 | awk '{print $2+$4+$6 \"%\"}'",
  ram  : "free | egrep 'Mem|buffers' | tr -d '\\n' | awk '{print $14*100/$7 \"%\"}'",
  disk : "df -lh | grep '% /$' | awk '{print $5}'",
  // processes : 'ps -ewwwo %cpu,%mem,comm | sort -n | tail -10 | tr "\\n" "@"'
  processes : "top -bn 1 -d 0.1 -c | grep -v 'top -bn' | grep '^\\s\\?[0-9]' | awk '{print $9 \" \" $10 \" \" $1 \" \" $12 \" \" $13}' | sort -n | tail -10 | tr '\\n' '@'"
}
...
var poll_command = function(interval) {
  ...
  list.push('echo $host_cpu == $host_ram == $host_disk == $host_processes');
...
Remote.prototype.poll = function(interval) {
  ...
  var parse = function(data) {
    ...
    self.process_list  = parseProcesses(split[3]);
